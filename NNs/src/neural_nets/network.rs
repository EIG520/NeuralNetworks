...
use crate::neural_nets::activations::*;
use neural_nets::network::rand::Rng;
use crate::model::*;
...
impl TNode {
    pub fn gen_val_from_inputs(&self, inputs: &[f32]) -> f32 {
        inputs.iter().enumerate().map(|(i,f)| f * self.weights[i]).sum()
    }
    pub fn forward_pass<T: Activation>(&mut self, inputs: &[f32], act: T) {
...
impl<T: Activation> TLayer<T> {
    pub fn step(&self, input: &[f32]) -> Vec<f32> {
        self.nodes.iter().map(
    ...
    fn gen_gradients_from_expected(&mut self, expected: &[f32]) {
        for (i, item) in expected.iter().enumerate().take(self.nodes.len()) {
            let node = &mut self.nodes[i];
            node.gradient = 2.0 * (node.value - item);
        }
    }
    ...
    pub fn set_values(&mut self, to: &[f32]) {
        for (val, node) in to.iter().zip(self.nodes.iter_mut()) {
            node.value = *val;
        }
    }
...
impl TrainableFFNet {
    ...
    pub fn forward_pass(&mut self, inputs: &[f32]) {
    ...
    pub fn backward_pass(&mut self, expected_outputs: &[f32]) {
    ...
    pub fn train_one_case_stochastic_gradient_descent(&mut self, inputs: &[f32], expected_outputs: &[f32], lr: f32) {
        self.forward_pass(inputs);
        self.backward_pass(expected_outputs);
        self.move_by_gradient(lr);
    }

    pub fn train_one_case_adam(&mut self, inputs: &[f32], expected_outputs: &[f32], alpha: f32, beta_1:f32, beta_2: f32, epsilon: f32) {
    ...
    pub fn train_on(&mut self, inputs: &[Vec<f32>], expected_outputs: &[Vec<f32>]) {
        for (input,expected) in inputs.iter().zip(expected_outputs) {
